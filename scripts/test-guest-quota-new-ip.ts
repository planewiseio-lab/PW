...
console.log("🎭 Guest Quota Test with New IP");
console.log("===============================");
console.log(`Timestamp: ${new Date().toISOString()}`);
console.log();

async function testGuestQuotaNewIP() {
  const baseUrl = "http://localhost:3000";

  console.log("🔍 Testing Guest Quota System with completely new IP...");
  console.log();
  ...
  try {
    const response = await fetch(`${baseUrl}/api/aircraft/lookup`, {
      ...
      headers: {
        ...
        "x-forwarded-for": "10.0.0.100", // Completely new IP
      },
  ...
  try {
    const response = await fetch(`${baseUrl}/api/aircraft/lookup`, {
      ...
      headers: {
        "Content-Type": "application/json",
        "x-forwarded-for": "10.0.0.100", // Same IP
      },
