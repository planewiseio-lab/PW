...
/**
 * Script pour tester la limite de crédits
 * Crée un utilisateur avec 3 crédits et teste le cron job
 */
...
async function testCreditLimit() {
  console.log("🧪 Testing Credit Limit Logic");
  console.log("==============================");

  const testUserId = "test-limit-user";

  try {
    // 1. Créer un utilisateur avec 3 crédits
    console.log("\n1. Creating user with 3 credits...");
    await createTestUserWithCredits(testUserId, 3);
    ...
    // 3. Exécuter le cron job (force pour contourner l'idempotence)
    ...
    console.log(`Expected: 5 credits (3 + 2)`);
    console.log(`Actual: ${finalBalance?.credits ?? 0} credits`);

    if (finalBalance?.credits === 5) {
      console.log("✅ Test PASSED: Credits correctly limited to 5");
    } else {
      console.log("❌ Test FAILED: Credits not limited correctly");
    }
  } catch (error) {
